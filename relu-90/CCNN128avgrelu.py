...
x1 = Dense(2*2*4,activation='relu')(decoder_input)
...
log_csv=CSVLogger('2000epochsrelu.csv',separator=',', append=False)
...
Verror=np.zeros((16384,1))
for i in range(16384):
    Uerror[i,:]=np.linalg.norm(Ureal[i,22:23]-Urec[i,22:23])
    Verror[i,:]=np.linalg.norm(Vreal[i,22:23]-Vrec[i,22:23])
    #Uerror[i,:]=np.linalg.norm(Ureal[i,44:45]-Urec[i,44:45])
    #Verror[i,:]=np.linalg.norm(Vreal[i,44:45]-Vrec[i,44:45])
    #Uerror[i,:]=np.linalg.norm(Ureal[i,67:68]-Urec[i,67:68])
    #Verror[i,:]=np.linalg.norm(Vreal[i,67:68]-Vrec[i,67:68])
    #Uerror[i,:]=np.linalg.norm(Ureal[i,89:90]-Urec[i,89:90])
    #Verror[i,:]=np.linalg.norm(Vreal[i,89:90]-Vrec[i,89:90])
    #Uerror[i,:]=np.linalg.norm(Ureal[i,119:120]-Urec[i,119:120])
    #Verror[i,:]=np.linalg.norm(Vreal[i,119:120]-Vrec[i,119:120])
